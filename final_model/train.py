"""
Image2GPS: Training script for ResNet50 model with grayscale augmentation.

Configuration:
- Backbone: ResNet-50
- Epochs: 50
- Augmentation: Grayscale only (best performing)
"""
...
def train(args):
    ...
    output_dir = os.path.join(args.output_dir, f"resnet50_{timestamp}")
    ...
    improvement = (1 - test_metrics['rmse']/test_baseline_metrics['rmse'])*100
    print(f"Improvement over baseline: {test_baseline_metrics['rmse'] - test_metrics['rmse']:.2f}m ({improvement:.1f}%)")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description='Train ResNet50 Image2GPS Model')
    ...
    print("=" * 70)
    print("Image2GPS Final Model Training (ResNet50 + Grayscale)")
    print("=" * 70)
